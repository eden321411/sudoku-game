...
function solveBoard(mat, row, col) {
    ...
    for (var num = 1; num <= 9; num++) {
        if (numberIsGood(mat, row, col, num)) {
            ...
            // the recursion flow failed , we need delete this number for back traking   
            mat[row][col] = 0;
        }
    }
...
//check if the number can be plaeced in the current cell (returns true/false)
...
//check if the number is alrady fonde in the row 
...
//check if the number is alrady fonde in the column 
...
//check if the number is alrady fonde in the square 
//   -------------------------
//   |   0   |   1   |   2   | 
//   -------------------------
//   |   3   |   4   |   5   |
...
//dril random numbers for the first row in the mat (will help us to genrate bigger range of different boards)
...
function chekIfSudokuSolutionIsRight(arrDeleteCells, solvingTime) {
    ...
    if (correctAnsCounter == arrDeleteCells.length) {
        ...
        window.location.href = '../theEnd/theEndWinner.html';
    }
    else {
        localStorage.setItem('mistakes', arrDeleteCells.length - correctAnsCounter);
        window.location.href = '../theEnd/theEndLoser.html';
    }
}
// localStorage.removeItem('topFiveWinnersTable'); //clear winners table from storage


//get the value of the mistakes
var currentGameTime = localStorage.getItem('gameTime');
